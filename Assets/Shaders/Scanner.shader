...
Shader "Custom/Water" {
	...
	SubShader {
		...
		frag_Output frag( frag_Input input )
		{
        	...
        	if (dot(normalDirection, lightDirection) > 1.0)  // Angle between normal and light
        	{
			...
			output.color = grabPassColor * output.color;

			// Check depthbuffer
			float sceneZ = LinearEyeDepth (tex2Dproj(_CameraDepthTexture, UNITY_PROJ_COORD(input.scrPos)).r);	// world space

			float partZ = input.scrPos.z * input.scrPos.z; // dist to camera working but scale is not linear
			float3 cameraToVert = vert - _WorldSpaceCameraPos;
			float depth = length(cameraToVert);

			float diff = (abs(depth - partZ)) / _Test2;
